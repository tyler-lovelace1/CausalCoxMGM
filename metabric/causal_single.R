...
train <- read.csv(paste0('data/metabric.rna',ifelse(erstatus=='all','', paste0('.',erstatus)),'.full.csv'), header=T, row.names=1)
...
if (mgmFlag) {
    ...
    ## pdf(paste0('steps.instabs.metabric.rna.',splitstatus,'.',erstatus,'.full.pdf'), width=5, height=5)
    ...
    pdf(paste0('solution.path.metabric.rna.',splitstatus,'.',erstatus,'.full.pdf'), width=5, height=5)
    plot(ig.path)
